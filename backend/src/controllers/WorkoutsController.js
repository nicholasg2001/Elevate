...
//Separate file in case I destroy everything :)
const getTopWorkouts = async (req, res) => {
    try {
        const workouts = await db.any("SELECT * FROM workouts ORDER BY workout_id LIMIT 6");
...
const getWorkoutsByMuscle = async (req, res) => {
    ...
    try {
        const workouts = await db.any("SELECT * FROM workouts WHERE muscle = $1", [muscle]);
        res.status(200).json(workouts);
    } catch (error) {
        console.error("Workouts Error:", error);
...
const getWorkoutByName = async (req, res) => {
    ...
    try {
        const workout = await db.oneOrNone("SELECT * FROM workouts WHERE name = $1", [workoutName]);
...
const getWorkoutsByType = async (req, res) => {
    ...
    try {
        const workouts = await db.any("SELECT * FROM workouts WHERE type = $1", [type]);
        res.status(200).json(workouts);
    } catch (error) {
        console.error("Workouts Error:", error);
        res.status(500).json({ error: "Internal Server Error" });
...
const getWorkoutsByDifficulty = async (req, res) => {
    ...
    try {
        const workouts = await db.any("SELECT * FROM workouts WHERE difficulty = $1", [difficulty]);
        res.status(200).json(workouts);
    } catch (error) {
        console.error("Workouts Error:", error);
        res.status(500).json({ error: "Internal Server Error" });
